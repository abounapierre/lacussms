 ...
 * @author SATELLITE
 */
public class PrintReportMandatPDF {
    private Document document = null;
    private final Font headerFont;
    private boolean status = false;
    private final LacusSmsService serviceManager;

    public PrintReportMandatPDF(String path, Date dateDeb, Date dateFin, LacusSmsService service) throws FileNotFoundException {
        ...
        try {
            ...
            table.setWidths(new float[]{2f, 3f, 10f, 4f, 4f, 6f, 4f, 4f});
            ...
            for (Message msg : serviceManager.getMessageFromPeriode(dateDeb, dateFin)) {
                ...
                table.addCell(cell);
                cell = new PdfPCell(new Phrase(Long.toString(msg.getBkEve().getCli().getPhone()), headerFont));
                table.addCell(cell);
                cell = new PdfPCell(new Phrase(msg.getSendDate().toString(), headerFont));
                table.addCell(cell);
                cell = new PdfPCell(new Phrase(msg.getBkEve().getCli() == null ? "" : msg.getBkEve().getCli().toString(), headerFont));
                table.addCell(cell);
                cell = new PdfPCell(new Phrase(msg.getBkEve().getCompte() == null ? "" : msg.getBkEve().getCompte().toString(), headerFont));
                table.addCell(cell);
                cell = new PdfPCell(new Phrase(msg.getBkEve().getBkAgence() == null ? "" : msg.getBkEve().getBkAgence().getNoma(), headerFont));
                table.addCell(cell);
                i++;
            }
            document.add(table);
